...
def click(btn):
    ...
    else: display.set(current + btn if current
                      != "0" else btn)
...
for i, row in enumerate(buttons):
    ...
    for j, btn in enumerate(row):
        color = ( "#f39c12" if btn in "+-*/="
              else "#3498db" if btn == "C" else "#34495e")
        btn_widget = tk.Label(frame, text=btn, font=(
            "Arial", 14, "bold"), bg=color, fg="white",
            width=5, height=2, relief="flat")
        btn_widget.bind("<Button-1>",
             lambda e, b=btn: click(b))
